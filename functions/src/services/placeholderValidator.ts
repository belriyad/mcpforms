...
export const placeholderValidator = {
   ...
   */
  validateField(field: PlaceholderField): { valid: boolean; errors: string[] } {
    const errors: string[] = [];

    // Validate field_key format
    if (!FIELD_KEY_REGEX.test(field.field_key)) {
      errors.push(
        `Invalid field_key "${field.field_key}": must match ^[a-z0-9_]{2,64}$ (lowercase alphanumeric + underscore, 2-64 chars)`
      );
    }

    // Validate type
    if (!VALID_TYPES.includes(field.type as any)) {
      errors.push(
        `Invalid type "${field.type}": must be one of ${VALID_TYPES.join(', ')}`
      );
    }
  ...
  validateSchema(fields: PlaceholderField[]): ValidationResult {
    ...
    for (const field of fields) {
      ...
      if (field.locations.length > 0) {
        const hasSpecificLocation = field.locations.some(
          loc => loc.page || loc.section || loc.xpath || loc.anchor
        );
   ...
   */
  detectCollisions(
    globalSchema: PlaceholderField[],
    overrideSchema: PlaceholderField[]
  ): Array<{ field_key: string; reason: string }> {
    const collisions: Array<{ field_key: string; reason: string }> = [];
