...
export const documentGenerator = {
  async generateDocuments(data: { intakeId: string; regenerate?: boolean; useAI?: boolean }): Promise<ApiResponse<{ artifactIds: string[] }>> {
    try {
      ...
      if (useAI) {
        try {
          ...
          const aiResult = await documentGeneratorAI.generateDocumentsFromIntake(intakeId, regenerate);
