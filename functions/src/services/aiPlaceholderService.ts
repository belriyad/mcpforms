...
export const aiPlaceholderService = {
   ...
   */
  async suggestPlaceholders(
    templateContent: string,
    fileType: 'docx' | 'pdf',
    existingPlaceholders?: PlaceholderField[]
  ): Promise<AIPlaceholderSuggestion> {
    try {
...
Return structured JSON with all identified fields.`;
      ...
      const response = await openai.chat.completions.create({
        ...
        response_format: {
          ...
          json_schema: {
            ...
            strict: true,
            schema: PLACEHOLDER_SUGGESTION_SCHEMA as any
          }
        },
        temperature: 0.2,
        max_tokens: 4096
      ...
      const suggestion: AIPlaceholderSuggestion = JSON.parse(content);

      console.log(`✅ [AI-PLACEHOLDER] Suggested ${suggestion.fields.length} fields`);
      console.log(`💰 [AI-PLACEHOLDER] Tokens used: ${response.usage?.total_tokens}`);

      // Post-validation (additional safety layer)
      const validation = placeholderValidator.validateSchema(suggestion.fields);
      if (!validation.valid) {
...
Return structured JSON.`;
      ...
      console.log(`✅ [AI-CLAUSE] Generated clause with ${result.new_placeholders?.length || 0} new placeholders`);
      console.log(`💰 [AI-CLAUSE] Tokens used: ${response.usage?.total_tokens}`);

      // Validate new placeholders
      if (result.new_placeholders && result.new_placeholders.length > 0) {
        const validation = placeholderValidator.validateSchema(result.new_placeholders);
        if (!validation.valid) {
          result.warnings = result.warnings || [];
          result.warnings.push(...validation.errors.map((e: any) => e.message));
        }

        // Check for collisions with existing placeholders
        const collisions = placeholderValidator.detectCollisions(existingPlaceholders, result.new_placeholders);
