...
const getOpenAIClient = () => {
  ...
  let apiKey = process.env.OPENAI_API_KEY || functions.config().openai?.api_key;
...
${text.substring(0, 6000)}`
  ...
  async processUploadedTemplate(data: { templateId: string; filePath: string }): Promise<ApiResponse<{ message: string }>> {
    try {
      ...
      const result = await templateParser.extractFieldsAndInsertionPoints(extractedText);
  ...
  async identifyInsertionPoints(text: string, openaiClient: any): Promise<any[]> {
    try {
...
${text}`
      ...
      console.log('✅ AI identified insertion points:', result.insertionPoints?.length || 0);
  ...
  async extractFieldsAndInsertionPoints(text: string): Promise<{ fields: FormField[], insertionPoints: any[] }> {
    try {
